...
// https://www.codeproject.com/Articles/415070/Dynamic-Type-Description-Framework-for-PropertyGri - newer version
//
...
namespace DynamicTypeDescriptor
{
    ...
    public class DynamicCustomTypeDescriptor : CustomTypeDescriptor
    {
        ...
        public void PopulateProperty(string propertyName, string[] values, bool addBlankIfEmpty = true, int numOfItemsToBeBrowsable = 1)
        {
            CustomPropertyDescriptor cpd = GetProperty(propertyName);
            cpd.StatandardValues.Clear();
            cpd.PropertyFlags |= PropertyFlags.ExclusiveStandardValues;
        ...
        public CustomPropertyDescriptor GetProperty<T>(Expression<Func<T>> propertyLambda)
        {
            var me = propertyLambda.Body as MemberExpression;
            if (me == null) throw new ArgumentException("You must pass a lambda of the form: '() => Class.Property' or '() => object.Property'");

            CustomPropertyDescriptor cpd = m_pdl.FirstOrDefault(a => String.Compare(a.Name, me.Member.Name, true) == 0);
            return cpd;
        }
